...
* The Publish-subscribe style :white_check_mark:
...
* The price for high scalability and availability is the need to have several replicas of the files to be shared :white_check_mark:
...
* Tiers, Shared-data, Service-oriented architecture, and Client-server :white_check_mark:
...
* Install style. :white_check_mark:
...
* None of the above.
* Modules are allocated to persons and teams. :white_check_mark:
...
* A install view.
* A deployment and a work assignment view. :white_check_mark:
...
* Analysing the performance of the system. :white_check_mark:
...
* Install style.
* Work assignment style.
* Implementation style. :white_check_mark:
...
* That is the recommended solution if there is a control flow that involves the choreography of both components :white_check_mark:
...
* The Communicating Processes style :white_check_mark:
...
* Can use a Service Registry to improve transparency of location of service providers. :white_check_mark:
...
* It is possible to integrate a new data accessor without changing the other data accessors. :white_check_mark:
...
* Scalability of read requests, because it is easy add more repositories to where reads are distributed, though there may be some level of inconsistency. :white_check_mark:
...
* Testability.
* Availability.
* Availability and Performance. :white_check_mark:
...
* All components may execute in all machines :white_check_mark:
...
* A shared-data style.
* A blackboard style.
* Both, client-server and shared-data styles. :white_check_mark:
...
* Pipes-and-Filters
* Client-server
* Dynamic Creation and Destruction :white_check_mark:
...
* The sharing of data is done using a service-oriented architecture. :white_check_mark:
...
* It is useful for system administrators. :white_check_mark:
...
* Service-oriented architecture, Shared-data, Client-server and Peer-to-peer. :white_check_mark:
...
* If there is some technology available that implements the complex connectors it is not necessary to document their decomposition. :white_check_mark:
...
* To facilitate the interaction among heterogeneous components that use distinct communication protocols :white_check_mark:
...
* A component is an instance and a view can have several instances of the same component type. :white_check_mark:
...
* It is preferable to only design views that do not use complex connectors to increase understandability.
* Whenever possible it should be avoided to use complex connectors because developers have difficult to know how to implement them.
* If there is some technology available that implements the complex connectors, according to its expected qualities, it is not necessary to document their decomposition. :white_check_mark:
...
* The Shared-Data style :white_check_mark:
...
* The Client-Server style
* The Publish-subscribe style
* The Pipes-and-filters style :white_check_mark:
...
* The Shared-Data style
* The Publish-subscribe style
* The Client-Server style :white_check_mark:
...
* She can define a variant of this style with asynchronous communication by allowing the client to register callbacks that the server calls at specific times. :white_check_mark:
...
* Peer-to-Peer.
* Client-Server.
* Pipe-and-Filter. :white_check_mark:
...
* The required quality associated with the connector is supported by existing and well-know technology. :white_check_mark:
...
* Restrict the communication between components because, for instance, a group of components should be located in the same hardware. :white_check_mark:
...
* The *Client-Server* style
* The *Publish-subscribe* style
* The *Peer-to-Peer* style :white_check_mark:
...
* Tiers to represent scalability. :white_check_mark:
...
* The web browser, o web server, and the data base :white_check_mark:
...
* A component can subscribe to events :white_check_mark:
...
* May, on another view of the system, be represented by a set of components and connectors :white_check_mark:
...
* It helps on the configuration of systems. :white_check_mark:
...
* The layered view to deal with the aspects of portability. :white_check_mark:
...
* Amazon Silk is more convenient for mobile devices because it does most of the computation in the cloud. :white_check_mark:
...
* Layered.
* Data model.
* Decomposition. :white_check_mark:
...
* Memcached can be considered a sub-module of the Store Graphs module. :white_check_mark:
...
* We must use various different views, both of the component-and-connector and the allocation viewtypes :white_check_mark:
...
* A single view of the communicating processes style. :white_check_mark:
...
* an ACID transaction occurs in the invoked application and ACID transactions in the other involved applications will eventually occur later :white_check_mark:
...
* The system represented in the right part of the figure tends to have good modifiability :white_check_mark:
...
* The measure of the response is throughput :white_check_mark:
...
* A layered architecture, where the access to the various sources is the responsibility of the bottommost layer :white_check_mark:
...
* The domain logic layer was implemented with the Transaction Script pattern :white_check_mark:
...
* An availability scenario of the *Graphite* system. :white_check_mark:
...
* She should try to split the system in parts in order to isolate the complex business logic and use the two architectural approaches accordingly. :white_check_mark:
...
* At least two performance scenarios of the *Graphite* system. :white_check_mark:
...
* Usability scenario.
* Business scenario. :white_check_mark:
...
* The stimulus and the response should be always present. :white_check_mark:
...
* Enumerates, for each kind of quality attribute, all the possible types of source of stimulus, stimulus, etc. :white_check_mark:
...
* The functional requirements have a large impact on the definition of views of the module viewtype because they are used to define the high cohesion and low coupling of modules :white_check_mark:
...
* Architecture is the design that gets harder to change as development progresses :white_check_mark:
...
* Limit event response
* Maintain multiple copies of computation
* Maintain multiple copies of data :white_check_mark:
...
* This view highlights the scalability of `upload` and `dowload` operations, and of storage. :white_check_mark:
...
* May be associated to other tactics to deal with a single stimulus. :white_check_mark:
...
* Write two scenarios on performance :white_check_mark:
...
* The separation of write and retrieval services allows them do scale independently :white_check_mark:
...
* Defer Binding :white_check_mark:
...
* Martin Fowler complains about this definition because the early decisions are not necessarily the right ones. :white_check_mark:
...
* Increasing performance and decreasing availability :white_check_mark:
...
* This shared understanding is what distinguishes architecture from design. :white_check_mark:
...
* In the mapping between layers of the system and the components where they execute :white_check_mark:
...
* the *Deployment* and *Layers* styles :white_check_mark:
...
* Tries to guarantee that the final system will have the qualities aimed by the architecture. :white_check_mark:
...
* Peer-to-Peer
* Uses
* Communicating Processes :white_check_mark:
...
* It is necessary to use views of the three viewtypes. :white_check_mark:
...
* Decomposition to express the services interfaces :white_check_mark:
...
* *Decomposition* and *Implementation* views :white_check_mark:
...
* Client-server in the first case and Peer-to-peer in the second. :white_check_mark:
...
* We have to use a Repository component-and-connector style. :white_check_mark:
...
* Create a decomposition where there is a module corresponding to the Windows OS and another one for the Mac OS X, each one responsible for containing the OS-specific code. :white_check_mark:
...
* Implementation :white_check_mark:
...
* The Client-Server style.
* The Communicating Processes style.
* The Service Oriented Architecture style. :white_check_mark:
...
* The stimulus is an omission and the tactic is retry :white_check_mark:
...
* All of the above
* Views of the Module viewtype :white_check_mark:
...
* The utility tree covers all the significant qualities the system has to address. :white_check_mark:
...
* Deployment views
* Implementation views :white_check_mark:
...
* A component may execute code from different modules :white_check_mark:
...
* Generalisation to represent an abstraction common to all interfaces and keep API-specific details in child modules :white_check_mark:
...
* Both in a component-and-connector and the Deployment views :white_check_mark:
...
* The execution of the previously existing layers is split between the two new tiers, and new intermediate layers may be needed :white_check_mark:
...
* They have to process very large amounts of data in each request :white_check_mark:
...
* Use an intermediary.
* Refactor.
* Restrict dependencies. :white_check_mark:
...
* This change manifests itself on the relationship between the system's modules and components :white_check_mark:
...
* It is necessary to change the connector between the web clients and the web servers, in the component-and-connector view, to show the semantics that is provided by the load-balancer :white_check_mark:
...
* A Uses view
* A Decomposition view :white_check_mark:
...
* The Decomposition and the Work Assignment styles :white_check_mark:
...
* To support high scalability the request of `User 1` needs to be decomposed into a request to only one of the aggregate instances and the processing in the other aggregate occurs in the background :white_check_mark:
...
* All viewtypes may be necessary :white_check_mark:
...
* The interaction between peers is symmetric. :white_check_mark:
...
* The *Client-Server* style
* The *Layers* style :white_check_mark:
...
* This view fulfills a modifiability scenario, which states about the cost of adding a new source of information to the system. :white_check_mark:
...
* A module
* Both, a module and a component :white_check_mark:
...
* The *decomposition* and *uses* styles, which allow us to show how dependent a certain module is of other parts of the system :white_check_mark:
...
* Is an extension of the Client-Server architectural style.
* Defines tiers as components.
* Restricts the communication between components because, for instance, a group of components should be located in the same hardware. :white_check_mark:
...
* The presentation logic layer and how it relates with the underlying layer changed :white_check_mark:
...
* Shared-data style. :white_check_mark:
...
* The communicating processes style and the dynamic reconfiguration style. :white_check_mark:
...
* You may need to use views of the three viewtypes :white_check_mark:
...
* Only depend on the type of events :white_check_mark:
...
* In Amazon Silk a request for a web page corresponds to requesting a service from the amazon cloud. :white_check_mark:
...
* It is useful for the project managers. :white_check_mark:
...
* Even though each viewtype addresses different aspects of a system, there are relationships among all of them :white_check_mark:
...
* They are both components :white_check_mark:
...
* Amazon Silk predicts accesses based on the information gathered for all Silk users. :white_check_mark:
...
* A module
* A layer
* A grouping of components :white_check_mark:
...
* The main quality this style addresses is interoperability. :white_check_mark:
...
* A decomposition view, a view of the component-and-connector viewtype and a deployment view. :white_check_mark:
...
* Differ on the emphasis on production and development phases of the software process :white_check_mark:
...
* Communicating processes, shared-data, service-oriented architecture, and peer-to-peer :white_check_mark:
...
* Identify what should be the common and specific parts of the module responsible for the interaction with the external sources, before interacting again with the stakeholders :white_check_mark:
...
* Becomes unavailable for banks if there is a fault in the hardware of service server (srv-opc) :white_check_mark:
...
* The attachment between components and connectors only depends on their ports and roles types. :white_check_mark:
...
* In the view there are multiple instances of the `Queue` component. :white_check_mark:
...
* Service-oriented architecture, Shared-data, Peer-to-peer, and Client-server :white_check_mark:
...
* It is possible to have redundant servers. :white_check_mark:
...
* To replace the machine used to run the server component by a more powerful machine that meets the new performance requirements, keeping only a server component running :white_check_mark:
...
* Allows the creation of checkpoints using the information that it gradually receives from the *NameNode* :white_check_mark:
...
* The application continues to have a three-tiered architecture, where one of the tiers is now the HDFS system :white_check_mark:
...
* Google Chrome predictor takes into consideration the amount of available cache. :white_check_mark:
...
* They are not implemented by a usability tactic :white_check_mark:
...
* Scalability of the Image File Storage in terms of the storage capacity :white_check_mark:
...
* To allow testing and validating the software architecture in the early development stages :white_check_mark:
...
* Should be captured in scenarios, as the requirements for quality attributes, and be taken into account in the design of the software architecture :white_check_mark:
...
* The Decomposition style :white_check_mark:
...
* The solution where the application is responsible for the eviction has better availability :white_check_mark:
...
* Is an iterative process where architectural designs are proposed as hypothesis and tested. :white_check_mark:
...
* Buffering can be considered a sub-module of the Store Graphs module. :white_check_mark:
...
* A module interface cannot be replicated but component ports can. :white_check_mark:
...
* It does not depend on a proprietary service :white_check_mark:
...
* This law can be seen as an example of the architecture influence cycle :white_check_mark:
...
* The stimulus for scenarios of the Security quality. :white_check_mark:
...
* The book definition also considers that the properties are externally visible because they are used for reasoning by the stakeholders. :white_check_mark:
...
* The most important requirements (both functional and qualities) that the system must achieve :white_check_mark:
...
* There is a interface delegation relation between the `read` port of `Queue` and the `query` port of `Carbon`. :white_check_mark:
...
* Tiers to express that different applications define their own contexts :white_check_mark:
...
* Development because it is not possible to do incremental development :white_check_mark:
...
* The availability guarantee may be given by the usage of an adequate connector between the HDFS Client and the DataNodes :white_check_mark:
...
* The location information is correctly included with a probability of 99.99% is the response measure. :white_check_mark:
...
* It allows high scalability because it is possible the implement transactions associated to each one of the aggregates :white_check_mark:
...
* Represent different architectural qualities and they may not be all represented in a single view :white_check_mark:
...
* Facilitate the communication among the stakeholders such that they can decide on what are the architecturally significant requirements. :white_check_mark:
...
* This view highlights the scalability of `upload` and `dowload` operations.
* This view highlights the performance of the `download` operations. :white_check_mark:
...
* Uses prefetching
* The *browser* needs to make less requests to the server :white_check_mark:
...
* Only a small set of functionalities are transactional :white_check_mark:
...
* The exchange of information is the stimulus. :white_check_mark:
...
* A request for a web page corresponds to requesting a service from the amazon cloud. :white_check_mark:
...
* It is an advantage for programmers that the transactional behavior is transparently provided :white_check_mark:
...
* A module view :white_check_mark:
...
* The Merge component executes the modules merge and stdio. :white_check_mark:
...
* Have low latency
* Allow many simultaneous users
* May be easily changed to increase their storage capacity :white_check_mark:
...
* The decomposition view to include a module for the synchronization responsibilities :white_check_mark:
...
* The connectors only use the *stdio* module for their implementation. :white_check_mark:
...
* A new aspect view that includes a module with the responsibilities associated with the access control and that crosscuts some of the other modules :white_check_mark:
...
* This view shows that if is possible to scale differently the `web tier` from the `EJB tier`. :white_check_mark:
...
* Communicating-Processes
* Tiers
* Client-Server and Shared-data :white_check_mark:
...
* It decouples applications developed for different organizations :white_check_mark:
...
* All layers are mapped to the browser component where the data access layer will contains, besides a module to access a local repository, modules to access external services. :white_check_mark:
...
* That the impact of integrating a new source is controlled by the interface of *Import DVD Info* Module. :white_check_mark:
...
* Use a passive redundancy tactic in the OPC (Order Processing Center) :white_check_mark:
...
* With a component-and-connector view, where the *load balancer* is a component of the system :white_check_mark:
...
* How to control the response to one or more stimulus :white_check_mark:
...
* In the asynchronous solution it is possible to provide an implement where the tasks being executed are finished without requiring the client to resubmitted them :white_check_mark:
...
* The decomposition was driven by a split module tactic. :white_check_mark:
...
* Allows the querying of a past state. :white_check_mark:
...
* Maintain user model.
* Maintain task model.
* Maintain system model. :white_check_mark:
...
* All the above. :white_check_mark:
...
* Which improves modifiability, because filters are decoupled through pipes. :white_check_mark:
...
* A performance scenario associated with the throughput of writing data points to disk. :white_check_mark:
...
* Performance and Availability for crashes of the Image File Storage component. :white_check_mark:
...
* An aggregate is usually loaded in its entirety from the database. :white_check_mark:
...
* The view results from the implementation of a support system initiative tactic. :white_check_mark:
...
* A Install view
* A Communicating processes view :white_check_mark:
...
* The asynchronous solution can support a larger number of simultaneous requests :white_check_mark:
...
* This solution allows the dimensioning of the number of activities (threads or processes) that run in the server, taking into consideration the server's hardware capacity, in order to have a efficient usage of the server's CPU :white_check_mark:
...
* If the OPC crashes the Consumer Website can continue to provide service in degraded mode. :white_check_mark:
...
* A new aspect view to include the responsibilities associated with the access control. :white_check_mark:
...
* Reads may not be consistent with the most recent write. :white_check_mark:
...
* Repository.
* Pipes-and-Filters.
* Client-server. :white_check_mark:
...
* The behavior described in the sentence can be represented in a view where the dynamic reconfiguration architectural style is used :white_check_mark:
...
* This ASR can easily be supported by the architecture because it has little effect in the architecture. :white_check_mark:
...
* It supports the concurrent access of data accessors. :white_check_mark:
...
* A module contains the code that can execute in several components and a component can execute the code of several modules :white_check_mark:
...
* The data model view in order to define generic entities that can be customized for different kinds of catalogs. :white_check_mark:
...
* It represents a relation between a component's port and a port of one of its internal components. :white_check_mark:
...
* The view addresses the scenario because it separates the modules that represent the interfaces a new business partner has to implement
* The view addresses the scenario because the `Consumer Website` module uses the `gwt` and `waf` modules
* The view addresses the scenario because it separates the `Consumer Website` module from the `OpcApp` module to allow the execution of the `Consumer Website` module in a component that can have multiple copies of computation :white_check_mark:
...
* Client-server, Repository and Publish-subscribe. :white_check_mark:
